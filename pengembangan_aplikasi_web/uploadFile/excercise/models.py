...
class Cashflow:
    def __init__(self, keterangan=None, nominal=None):
    ...
    def insertPengeluaran(self, nama_file):
        ...
        cursor.execute(
            "INSERT INTO UPLOAD_FILE (NAMA_FILE, TGL_UPLOAD) VALUES('%s', '%s')" % (nama_file, today)
        )
    ...
    def getCountDatainDatabase(self):
        self.openDatabase()
        cursor.execute(
            "SELECT COUNT(ID_FILE) FROM UPLOAD_FILE"
        )
